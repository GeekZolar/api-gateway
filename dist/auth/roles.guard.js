...
let RolesGuard = class RolesGuard {
    canActivate(context) {
        ...
        if (!user?.role) throw new _common.ForbiddenException('Missing user role');
        const role = user.role.toLowerCase().replace(/\s+/g, '-');
        const normalized = role === 'user' ? 'read-only' : role;
        if (!requiredRoles.includes(normalized)) {
            throw new _common.ForbiddenException('Insufficient permissions');
        }
        return true;
    }
